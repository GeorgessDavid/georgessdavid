...
const Button = ({ background, color, action, link, style, children }: ButtonProps) => {
    return (
        <button className={`${style === 'filled' ? `bg-(${background}) text-(${color}) border-2 border-transparent hover:bg-(--bg-primary) hover:text-(--text-accent) hover:border-(--text-accent)`
